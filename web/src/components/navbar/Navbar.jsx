...
function Navbar() {
  ...
  return (
      <nav id="main-navigation" className='md:w-[80%] mx-auto max-w-screen-xl md:mx-auto px-14 md:px-4 relative font-sans text-2xl flex items-center justify-between'>
          <Link to="/home#hero" onClick={() => setOpen(false)}  className='animated-underline z-50' aria-label='home' >Aurélie Nogueira</Link>
          <HamburgerMenu handleClick={ handleClick } open={open}/>
          <ul className={`${open ? 'absolute w-full top-0 left-0 px-14 my-4 py-12 z-40 bg-background shadow-[0_4px_6px_rgba(0,0,0,0.1)] md:shadow-none grid justify-start md:relative md:flex md:justify-end md:my-0 md:p-0 md:gap-20' : 'hidden shadow-none md:relative md:flex md:mt-0 md:p-0 md:gap-20'}`}>
